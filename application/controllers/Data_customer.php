...
class Data_customer extends CI_Controller
{
    ...
    {
        $data1['title'] = 'Janitra Admin - Data Customer';
        $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
        $data2['datauser'] = $this->data_customer_model->ambil_data()->result();
        ...
        $this->load->view('admin/content/data/data_customer', $data2);
    ...
    public function tambah_customer()
    {
        ...
        if ($this->form_validation->run() == false) {
            $data['title'] = 'Janitra Admin - Data Customer';

            $this->load->view('templates/admin_header', $data);
            $this->load->view('templates/admin_sidebar');
            $this->load->view('templates/admin_topbar');
            $this->load->view('admin/content/data/tambah_customer');
            $this->load->view('templates/admin_footer');
        } else {
            $result = $this->data_customer_model->tambah_data();
            if ($this->db->affected_rows() > 0) {
                $this->session->set_flashdata('success', 'Data berhasil disimpan');
            }
            echo "<script>window.location='".site_url('data_customer')."';</script>";
        }
    }

    public function hapus($id)
    {
        $this->data_customer_model->hapus($id);
    ...
    public function edit($id)
    {
        ...
        if ($this->form_validation->run() == false) {
            $data['title'] = 'Janitra Admin - Data Customer';
            $query = $this->data_customer_model->get($id);
            ...
            if ($query->num_rows() > 0) {
                ...
                $this->load->view('templates/admin_sidebar');
                $this->load->view('templates/admin_topbar');
                $this->load->view('admin/content/data/edit_customer', $data1);
                $this->load->view('templates/admin_footer');
            } else {
                echo "<script>alert('Data tidak ditemukan'); window.location='".site_url('data_customer')."';</script>";
            }
        } else {
            $post = $this->input->post(null, TRUE);
            $this->data_customer_model->edit_data($post);
            if ($this->db->affected_rows() > 0) {
                $this->session->set_flashdata('success', 'Data berhasil disimpan');
            }
            echo "<script>window.location='".site_url('data_customer')."';</script>";
